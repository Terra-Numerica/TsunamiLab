...
import json
import sys
sys.path.insert(0, '../LeapSDK/python3.5_project')
import Leap
		...
		if controller.is_connected:
			...
			if frame.is_valid:
				if on_connect:
					...
					await websocket.send(json.dumps(get_aspect_ratio(frame)))
					on_connect = False

				if frame.hands:
					#print('hands in frame')
					new_frame = get_frame(frame)
					await websocket.send(json.dumps(new_frame))
			PREV_FRAME = frame
